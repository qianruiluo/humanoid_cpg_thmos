...
void WalkVisualizer::publishIKDebug(WalkResponse response,
  ...
  tf2::convert(goal_state->getFrameTransform(l_sole_frame_), pose_left_result);
  ...
  tf2::convert(goal_state->getFrameTransform(r_sole_frame_), pose_right_result);
  ...
  Eigen::Vector3d l_transform = goal_state->getGlobalLinkTransform(l_sole_frame_).translation();
  Eigen::Vector3d r_transform = goal_state->getGlobalLinkTransform(r_sole_frame_).translation();
  ...
  tf2::convert(current_state->getGlobalLinkTransform(l_sole_frame_), pose_left_actual);
  ...
  tf2::convert(current_state->getGlobalLinkTransform(r_sole_frame_), pose_right_actual);
  ...
  l_transform = current_state->getGlobalLinkTransform(l_sole_frame_).translation();
  r_transform = current_state->getGlobalLinkTransform(r_sole_frame_).translation();
  tf2::convert(l_transform, tf_vec_left);
  tf2::convert(r_transform, tf_vec_right);
  if (response.is_left_support_foot) {
    support_off = trunk_to_support_foot.getOrigin() - tf_vec_left;
