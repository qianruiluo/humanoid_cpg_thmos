...
void WalkIK::init(moveit::core::RobotModelPtr kinematic_model) {
  legs_joints_group_ = kinematic_model->getJointModelGroup("Legs");   //moveit 获取规划组
...
bitbots_splines::JointGoals WalkIK::calculate(const WalkResponse &ik_goals) {
  ...
  // decide which foot is which   把tf2 Transform类型转换为geometry_msgs::Pose，存到left_foot_goal_msg里
  ...
  success = goal_state_->setFromIK(left_leg_joints_group_,    //对left_leg规划组进行逆运动学解算
                                   left_foot_goal_msg,        //传入trunk到left_foot的transformation
                                   ik_timeout_,
  ...
  goal_state_->copyJointGroupPositions(legs_joints_group_, joint_goals); //获取逆运动学解算后各关节的数值
...
void WalkIK::reset() {    //设置初始状态，需修改关节名字和数值
