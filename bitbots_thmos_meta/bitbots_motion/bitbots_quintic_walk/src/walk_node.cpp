...
namespace bitbots_quintic_walk {  //命名空间
...
WalkNode::WalkNode(const std::string ns) :
  ...
  //load MoveIt! model  使用moveit获取机器人运动学模型（RobotModel接口类）
...
// update walk engine response
  ...
  current_response_ = walk_engine_.update(dt);   //状态机 根据状态建立轨迹
  ...
  std::ofstream fout;
  fout.open("/home/mlq/2021Robocup/bitbots_meta/bitbots_motion/bitbots_quintic_walk/src/data/walking_motor_goals.csv", std::ios::app);
  for(int i = 0;i<command.positions.size()-1;i++)
  {
    fout<<command.positions[i]<<",";
  }
  fout<<command.positions[command.positions.size()-1]<<"\n";
  fout.close();
...
geometry_msgs::Pose WalkNode::get_left_foot_pose() {
  ...
  tf2::convert(goal_state->getGlobalLinkTransform(l_sole_frame_), pose);
